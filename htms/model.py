...
class Model():
            ...
            for i in np.arange(0, len(model.topics_models), 1):
                if str(type(model.topics_models[i])) == "<class 'Model.Model'>":
                    if model.topics_models[i].model_name == model_selected:
                        ...
                        self.add_to_father(
                            model_selected, model.topics_models[i])
        ...
        for i in np.arange(0, len(self.topics_models), 1):
            if str(type(self.topics_models[i])) == "<class 'Model.Model'>":
                if self.topics_models[i].model_name == old_name:
                    ...
                    self.topics_models[i].rename_child(
      ...
      * ident        - Character for make the enumeration
      """
        if route_model:
            ...
            if toPrint:
                print(Fore.GREEN + "[" + str(len(models_names) -
                      1) + "] " + Fore.WHITE + self.model_name)
            route_model = False
            self.print_model(models_names, models_paths,
                             route_model, ident, toPrint)
            ...
            for i in np.arange(0, len(self.topics_models), 1):
                if str(type(self.topics_models[i])) == "<class 'Model.Model'>":
                    ...
                    model_ident = ident * \
                        len(self.topics_models[i].father_models)
                    if toPrint:
                        print(Fore.GREEN + model_ident + "[" + str(len(models_names) - 1) + "] " + Fore.WHITE +
                              self.topics_models[i].model_name)
                    self.topics_models[i].print_model(
                        models_names, models_paths, route_model, ident, toPrint)
        if not toPrint:
            return models_names
        return
            ...
            for i in np.arange(0, len(self.topics_models), 1):
                if str(type(self.topics_models[i])) == "<class 'Model.Model'>":
                    ...
                    print(Fore.GREEN + model_ident + "[" + str(len(models_names) - 1) + "] " + Fore.WHITE +
                          self.topics_models[i].model_name)
                    self.topics_models[i].print_model(
                        models_names, models_paths, route_model, ident)
        return
        ...
        * topics_ids   - The selected topics' ids for creating each corresponding
        ...
        while True:
            try:
                print("")
                num_models = int(
                    input("How many submodels do you want to create? "))
                if (num_models < 1) or (num_models > range_ids):
                    print(
                        "Invalid number. The number of submodels must be between 1 and " + str(range_ids) + ".")
                else:
                    break
            except:
            ...
            while True:
                try:
                    ...
                    if (range_ids < 0) or (id > range_ids - 1):
                        print(
                            "Invalid number. The topic id must be a number between 0 and " + str(range_ids - 1) + ".")
                    else:
                        topic_ids.append(id)
                        break
                except:
    ...
    def set_topic_description(self, route_to_model, model_ids):
        """Sets the description for a set of topics. To do so, it asks the user to insert the description that he wants to give for each of the topics of a model.

        This method is only used for the command line application.

        Parameters:
        ----------
        * route_to_model - Route to the model
        * model_ids      - File where the topics with its description and chemical description 
                           are saved.
        """
            ...
            if str(type(self.topics_models[i])) == "<class 'Topic.Topic'>":
                while True:
                    try:
                        print("")
                        topic_name = str(
                            input("Topic " + str(self.topics_models[i].id_topic) + ": "))
    ...
    def set_one_topic_description(self, route_to_model, model_ids, topic_id, topic_name):
        """Same as "set_topic_description", nut only for one topic selected by the user. In this case, the name that is gonna be used as the topic's description is given by parameter, rather than being asked to the user through the command line.
        ...
        * model_ids      - File where the topics with its description and chemical description 
                           are saved.
        * topic_id       - Id of the topic whose "name" is going to be changed.
