# -*- coding: utf-8 -*-
"""
Created on Mon Aug  2 11:27:54 2021
...
sys.path.insert(1, 'C:\\Users\\lcalv\\OneDrive\\Documentos\\GitHub\\MusicalSpoonV3\\hierarchical_topic_model')
...
def get_characteristics_submodel(str_start, str_end, str_not_end, version, submodel_names, submodel_coherences, submodel_entropies, submodel_nr_documents,submodel_threshold):
    ...
    submodel_names_ = [i for i in submodel_names if (i.startswith(str_start) and i.endswith(str_end) and not i.endswith(str_not_end))]
...
def get_submodel_values(file, corpus):
    ...
    diagnostics_path = ((pathlib.Path(model.model_path)) / "diagnostics.xml").as_posix()
    tree = ET.parse(diagnostics_path)
    root = tree.getroot()
    entropies_root = [child.get("document_entropy") for child in root if child.tag == 'topic']
    ...
    for i in model.topics_models:
        if str(type(i)) == "<class 'Model.Model'>":
            ...
            # Get entropy
            diagnostics_path = ((pathlib.Path(i.model_path)) / "diagnostics.xml").as_posix()
            tree = ET.parse(diagnostics_path)
            root = tree.getroot()
            entropies = [child.get("document_entropy") for child in root if child.tag == 'topic']
...
def parse_to_results(results, id_model, submodel_names, submodel_coherences, submodel_threshold, submodel_entropies, submodel_nr_documents, model):
    ...
    for i in np.arange(0, model.num_topics, 1):
        ...
        results.append([submodel_coherences_, submodel_names_, submodel_entropies_, submodel_nr_docs_, submodel_thrs_, id_model])
...
for i in range(len(sources)):
    ...
    submodel_names, submodel_coherences, submodel_threshold, submodel_entropies, submodel_nr_documents, model = get_submodel_values(sources[i], corpus)
    results = parse_to_results(results, id_model, submodel_names, submodel_coherences, submodel_threshold, submodel_entropies, submodel_nr_documents, model)
    id_model +=1
df = pd.DataFrame(results, columns=['Coherences', 'Names', 'Entro', 'NrDocs', 'Thr', 'Model'])
...
for i in df.index:
    ...
    for j in range(len(row_names)):
        if(df["Thr"][i] == 0):
            results_all_all.append([df["Coherences"][i][j], df["Names"][i][j], df["Entro"][i][j], df["NrDocs"][i][j], df["Thr"][i], df["Model"][i]])
        else:
            results_all_all.append([df["Coherences"][i][j], df["Names"][i][j], df["Entro"][i][j], df["NrDocs"][i][j], df["Thr"][i][j], df["Model"][i]])
df2 = pd.DataFrame(results_all_all, columns=['Coherences', 'Names', 'Entro', 'NrDocs', 'Thr', 'Model'])
...
for topic_trained in iter_topics:
    for topic_from in np.arange(0, 5, 1):
        ...
        for thr in np.arange(0.1, 1, 0.1):
            ending = "{:.1f}".format(thr) + "_" + str(topic_trained) + "_topics"
            cohrs = df2[(df2.Thr > 0) & (df2.Names.str.endswith(ending)) & (df2.Names.str.startswith(starts))]["Coherences"]
            entro = df2[(df2.Thr > 0) & (df2.Names.str.endswith(ending)) & (df2.Names.str.startswith(starts))]["Entro"]
            nrdocs = df2[(df2.Thr > 0) & (df2.Names.str.endswith(ending)) & (df2.Names.str.startswith(starts))]["NrDocs"]
            final_results.append([thr, topic_from, topic_trained, cohrs.mean(), cohrs.std(), entro.mean(), entro.std(), nrdocs.mean(), nrdocs.std()])
...
df_final_results = pd.DataFrame(final_results, columns=['Thr', 'TopFrom', 'TrainTops', 'Cohr_m', 'Cohr_std', 'Entro_m', 'Entro_std', 'NrDocs_m', 'NrDocs_std'])
