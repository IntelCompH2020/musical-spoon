...
class Topic():
    ...
    def __init__(self, id_topic, betas, description, description_name):
    ...
    def save_betas(cls, file, topic_ids):
        ...
        for j in np.arange(0, len(topic_ids), 1):
            ...
            betas_norm = normalize(
                betas[:, np.newaxis], axis=0, norm='l1').ravel()
    ...
    def create_topics(num_topics, topic_word_weights, topic_keys):
        ...
        topic_keys_weight, description_all = Topic.save_desciptions(
            topic_keys, topic_ids)
        for i in np.arange(0, len(betas_all), 1):
            topic = Topic(i, betas_all[i], description_all[i], "")
            topics.append(topic)
