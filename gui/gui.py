...
***                                 GUI                                    ***
******************************************************************************
Module that specifies the main window class (UI_MainWindow) for the
GUI and inherits from QMainWindow. It loads the UI file in which the design of
the application has been implemented and adds to it visual appeal. Besides, it
carries out the controlling of all the widgets and the reception to the user’s
interaction.
"""
...
class UI_MainWindow(QtWidgets.QMainWindow):
    def __init__(self, version):
        ...
        self.infoButtonSelectSubmodel_4.setToolTip(
            MessagesGui.INFO_SELECT_SUBMODEL)
        self.infoButtoSelectTopicExpand_4.setToolTip(
            MessagesGui.INFO_SELECT_TOPIC_TO_EXPAND)
        self.infoButtonShowDescription.setToolTip(
            MessagesGui.INFO_SHOW_DESCRIPTION)
        self.infoButtonDiagnostics.setToolTip(MessagesGui.INFO_DIAGNOSTICS)
        self.infoButtonSelectModelDiagnostic.setToolTip(
            MessagesGui.SELECT_COMPARE)
        self.infoButtonDragTopicDiagnostic.setToolTip(
            MessagesGui.DRAG_COMPRARE)

        # CONFIGURE ELEMENTS IN THE "CONFIGURATION VIEW"
        ########################################################################
        self.treeViewSelectDataset.setContextMenuPolicy(
            QtCore.Qt.CustomContextMenu)
        self.treeViewSelectDataset.customContextMenuRequested.connect(
            self.context_menu)
        self.treeViewSelectDataset.doubleClicked.connect(
            self.clicked_select_dataset)
        ...
        self.pushButtonApplySettings.clicked.connect(
            self.apply_changes_settings)
        ...
        self.tableWidgetModelFiles.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str("")))
        self.tableWidgetModelFiles.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str("")))

        self.tableWidgetHTMVersion.doubleClicked.connect(
            self.clicked_changeHTM)

        # CONFIGURE ELEMENTS IN THE "SELECT ROUTE MODEL VIEW"
        ########################################################################
        self.column_listSelectModel.clicked.connect(
            self.clicked_selected_model)
        ...
        self.treeViewShowModelsToExpand_4.clicked.connect(
            self.clicked_model_to_change_description)
        self.buttonApplyChanges.clicked.connect(self.clicked_apply_changes)
        self.buttonResetDescriptions.clicked.connect(
            self.clicked_reset_changes)
        # DELETE SUBMODEL
        self.treeViewShowModelsToExpand_4.clicked.connect(
            self.clicked_selected_model_to_delete)
        self.pushButtonToDeleteSubmodel_4.clicked.connect(
            self.clicked_delete_submodel)

        # CONFIGURE ELEMENTS IN THE "EDIT MODEL VIEW"
        ########################################################################
        self.treeViewShowModelsToExpand_4.clicked.connect(
            self.clicked_model_to_expand_selected)
        ...
        ########################################################################
        self.treeWidgetSelectModelToSeeDescription.clicked.connect(
            self.clicked_select_model_to_see_description)
        self.pushButtonSeeDescription.clicked.connect(
            self.clicked_see_topics_description)
        ...
        ########################################################################
        self.treeWidgetSelectModelToDiagnostic.clicked.connect(
            self.clicked_showTopicsDragDiagnosis)
        self.pushButtonPlotDiagnosisGraph.clicked.connect(
            self.click_draw_diagnosis)
        self.pushButtonClearDiagnostics.clicked.connect(
            self.clear_tables_graph_diagnostics)
    ...
    def initialize_settings(self):
        ...
        self.tableWidgetGeneralSettings.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(first_menu_source_path)))
        self.tableWidgetGeneralSettings.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(first_menu_project_path)))
        self.tableWidgetMalletSettings.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(first_menu_mallet_path)))
    ...
    def set_default_settings(self):
        ...
        self.tableWidgetGeneralSettings.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(source_path)))
        self.tableWidgetGeneralSettings.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(project_path)))

        self.tableWidgetMalletSettings.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(mallet_path)))
        self.tableWidgetMalletSettings.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(optimize_interval)))
        self.tableWidgetMalletSettings.setItem(
            2, 1, QtWidgets.QTableWidgetItem(str(model_mallet)))
        self.tableWidgetMalletSettings.setItem(
            3, 1, QtWidgets.QTableWidgetItem(str(submodel_mallet)))

        self.tableWidgetOutputFiles.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(topic_state)))
        self.tableWidgetOutputFiles.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(topic_keys)))
        self.tableWidgetOutputFiles.setItem(
            2, 1, QtWidgets.QTableWidgetItem(str(doc_topics)))
        self.tableWidgetOutputFiles.setItem(
            3, 1, QtWidgets.QTableWidgetItem(str(topic_word_weights)))
        self.tableWidgetOutputFiles.setItem(
            4, 1, QtWidgets.QTableWidgetItem(str(model_ids)))
    ...
    def apply_changes_settings(self):
        ...
        new_optimize_interval = self.tableWidgetMalletSettings.item(
            1, 1).text()
        ...
        config.set('out-documents', 'topic_word_weights',
                   new_topic_word_weights)
        ...
        QtWidgets.QMessageBox.information(
            self, 'MusicalSpoon message', "Changes were saved in the configuration file.")
    ...
    def show_datasets(self):
        ...
        path = pathlib.Path(pathlib.Path(
            "\mallet").parent.absolute(), "mallet").as_posix()
    ...
    def clicked_select_dataset(self):
        ...
        self.statusBar().showMessage("'" + str(dataset_selected_path) +
                                     "' was selected as dataset.", 10000)
        ...
        self.tableWidgetGeneralSettings.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(dataset_selected_path)))
    ...
    def clicked_changeHTM(self):
        ...
        if r == 0:
            ...
            self.statusBar().showMessage(
                "The algorithm of the underlying HTM was changed to version 1.", 10000)
        elif r == 1:
            self.version = "v2"
            self.statusBar().showMessage(
                "The algorithm of the underlying HTM was changed to version 2.", 10000)
        ...
        if pathlib.Path(project_path, "models").is_dir():
            ...
            if models:
                for model_nr in np.arange(0, len(models), 1):
                    self.column_listSelectModel.insertItem(
    ...
    def clicked_delete_model(self):
        ...
        if not models:
            ...
            QtWidgets.QMessageBox.warning(
                self, 'MusicalSpoon message', "There are not models to delete.")
        else:
            if not self.column_listSelectModel.currentItem():
                QtWidgets.QMessageBox.warning(
                    self, 'MusicalSpoon message', "A model to delete must be selected first.")
                return
            else:
                model_to_delete_str = str(
                    self.column_listSelectModel.currentItem().text())
    ...
    def clicked_selected_model(self):
        ...
        self.tableWidgetModelFiles.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(model_selected)))
        self.tableWidgetModelFiles.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(persistence_selected)))
    ...
    def clicked_train_model(self):
        ...
        if not self.insertNumberTopicsModel.text():
            ...
            QtWidgets.QMessageBox.warning(
                self, 'MusicalSpoon message', "The number of topics must be a number.")
            return
        else:
        ...
        #################################################################################
        self.execute_in_thread(self.execute_to_train_model,
                               self.do_after_train, True)
    ...
    def do_after_train(self):
        ...
        self.tableWidgetModelFiles.setItem(
            0, 1, QtWidgets.QTableWidgetItem(str(new_model_name)))
        self.tableWidgetModelFiles.setItem(
            1, 1, QtWidgets.QTableWidgetItem(str(new_persistence)))
    ...
    def clicked_model_to_expand_selected(self):
        ...
        if not self.treeViewShowModelsToExpand_4.currentItem().text(0).lower().startswith("model") and \
            ...
            QtWidgets.QMessageBox.warning(
                self, 'MusicalSpoon message', "You must select an item. no an empty space.")
            return
        item = self.treeViewShowModelsToExpand_4.currentItem()
    ...
    def clicked_train_submodel(self):
        ...
        if not os.listdir(route_to_models):
            ...
            QtWidgets.QMessageBox.warning(
                self, 'MusicalSpoon message', "You must train the model first.")
        ...
        if not self.tableWidgetTrainSubmodel_4.item(r, 0):
            ...
            self.topic_to_expand = int(
                self.tableWidgetTrainSubmodel_4.item(r, 0).text())
        print(self.version)
        if self.version == "v2":
        ...
        if not self.InsertNumberTopicsSubmodel_8.text():
            ...
            self.num_training_topics = int(
                self.InsertNumberTopicsSubmodel_8.text())
        ...
        self.execute_in_thread(
            self.execute_to_train_submodel, self.do_after_train_submodels, True)
    ...
    def do_after_train_submodels(self):
        ...
        self.execute_in_thread(self.execute_get_topic_ids,
                               self.do_after_get_topicsIds_train_submodel, False)
    ...
    def do_after_get_topicsIds_train_submodel(self):
        ...
        for i in np.arange(0, len(self.list_description), 1):
            ...
            item_topic_description = QtWidgets.QTableWidgetItem(
                str(self.list_description[i]))
            self.tableWidgetSubmodelTrained_4.setItem(
    ...
    def clicked_see_topics_description(self):
        ...
        if item is None:
            ...
            if not topic_ids:
                ...
                QtWidgets.QMessageBox.warning(
                    self, 'MusicalSpoon message', "No model has been trained yet.")
                ...
                self.execute_in_thread(
                    self.execute_get_topic_ids, self.do_after_getIds_see_description, False)
    ...
    def clicked_model_to_change_description(self):
        ...
        if not topic_ids:
            ...
            self.execute_in_thread(
                self.execute_get_topic_ids, self.do_after_getIds_change_description, False)
    ...
    def clicked_apply_changes(self):
        ...
        """
        self.execute_in_thread(self.execute_apply_changes,
                               self.after_apply_changes, True)
    ...
    def clicked_reset_changes(self):
        ...
        submodel in both the pickle and file "topic_ids".
        """
        self.execute_in_thread(self.execute_reset_changes,
                               self.after_reset_changes, True)
    ...
    def clicked_plot_pyldavis(self):
        ...
        """
        self.inicio = time.time()
        self.model_to_plot = str(
            self.treeWidgetSelectModelToSeeDescription.currentItem().text(0))
        ...
        file_pldavis = pathlib.Path(
            route_to_model_plot_pyldavis, pldavis).as_posix()

        if not os.path.exists(file_pldavis):
            self.execute_in_thread(
                self.execute_to_get_pyldavis, self.after_show_plot_pyldavis, True)
        else:
    ...
    def clicked_plot_diagnosis(self):
        ...
        self.inicio = time.time()
        self.model_to_plot = str(
            self.treeWidgetSelectModelToSeeDiagnostics.currentItem().text(0))
        ...
        model_plot_pyldavis = model.look_for_model(self.model_to_plot)
        route_to_model_plot_pyldavis = model_plot_pyldavis.model_path
        file_diagnostics = pathlib.Path(
            route_to_model_plot_pyldavis, diagnostics).as_posix()
    ...
    def clicked_delete_submodel(self):
        ...
        """
        model_to_delete_str = str(
            self.treeViewShowModelsToExpand_4.currentItem().text(0))
        ...
        if button_reply_delete_model == QtWidgets.QMessageBox.Yes:
            ...
            if model_to_delete_str.startswith("model"):
                ...
                try:
                    ...
                    self.statusBar().showMessage(
                        "You must have trained a model first in order to delete it.", 10000)
                    QtWidgets.QMessageBox.warning(self, 'MusicalSpoon message',
            ...
            if not deleted:
                ...
                QtWidgets.QMessageBox.warning(
                    self, 'MusicalSpoon message', "There are no submodels to delete.")
                return
            else:
                QtWidgets.QMessageBox.information(self, 'MusicalSpoon message',
    ...
    def clicked_showTopicsDragDiagnosis(self):
        ...
        if self.treeWidgetSelectModelToDiagnostic.currentItem().text(0).lower == "models":
            QtWidgets.QMessageBox.warning(
                self, 'MusicalSpoon message', "You must select an item within 'models'.")
            return
    ...
    def click_draw_diagnosis(self):
        ...
        for i in np.arange(0, self.tableDragTo.rowCount(), 1):
            ...
            diagnostics_path = ((pathlib.Path(model_path)) /
                                "diagnostics.xml").as_posix()
            diagnostics_paths_id_all.append(
                [diagnostics_path, model_name, topic_id])

        x, y = plot_diagnostics(diagnostics_paths_id_all, measurement,
                                measurement2, text_xaxis, text_yaxis, text_title, figure_to_save)
