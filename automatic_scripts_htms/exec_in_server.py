...
***                             EXEC_IN_SERVER                             ***
******************************************************************************
A script to automatically generates hierarchical topic models according to
HTM-WS and HTM-DS and save the results in a pickle file.
"""
##############################################################################
#                                IMPORTS                                     #
##############################################################################
...
for htm in range(num_htm):
    ...
    # 2. Train root model
    route_to_persistence, model_name = train_model(
        model_path, model_name, num_topics_root)
        ...
        for i in np.arange(0, len(model.topics_models), 1):
            if str(type(model.topics_models[i])) == "<class 'Topic.Topic'>":
                ...
                for thr in np.arange(0.1, 1, 0.1):
                    start_time = timer.time()
                    train_save_submodels(model_path, route_to_persistence, model_name,
                                         model_name, topic_id, num_topics_sub[j], "v2", thr)
                    training_time = timer.time() - start_time
                    logger.debug("v2", "{:.1f}".format(thr), str(
                        num_topics_sub[j]), str(training_time))
                start_time = timer.time()
                train_save_submodels(model_path, route_to_persistence, model_name,
                                     model_name, topic_id, num_topics_sub[j], "v1", "")
                training_time = timer.time() - start_time
                logger.debug("v1", str(0), str(
                    num_topics_sub[j]), str(training_time))
