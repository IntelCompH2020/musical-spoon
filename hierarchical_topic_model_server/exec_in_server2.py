...
#                             VARIABLES                                      #
##############################################################################
#project_path = "/export/usuarios01/lbartolome/projectNIH"
#source_path = "/export/usuarios01/lbartolome/mallet-2.0.8/nih.txt" #"/mallet/cord19.txt"
#model_ids = "model_ids.txt"
#time = strftime("_%Y-%m-%d_%H-%M-%S", gmtime())
#num_topics_root = 8
#logging_file = "/export/usuarios01/lbartolome/projectNIH/root_" + str(num_topics_root) 

project_path = "D:\\project_cord_timings" #/projectNIH
source_path = "C:\\mallet\\cord19.txt" #"/mallet/cord19.txt"
model_ids = "model_ids.txt"
time = strftime("_%Y-%m-%d_%H-%M-%S", gmtime())
num_topics_root = [5,8,10,16,20,30]
#logging_file = "D:\\project_cord_timings\\log_root" 
...
def train_model(route_to_model, name, nr_topics):
    ...
    model.set_training_time(training_time)
    ...
    #                     topic_ok.append(k)
    #                     break
    #         topics.append(topic_ok) 
    
    # D = gensim.corpora.Dictionary(corpus)
    # # Calculate average coherence of the root model and per topic
    ...
    # coherences_per_topic = cm.get_coherence_per_topic()
    # print("coherences_per_topic", coherences_per_topic)
    # print("avg cohr", coherence)
    
    # model.set_avg_coherence(coherence)
    
    # for i in range(len(model.topics_models)):
    #     if str(type(model.topics_models[i])) == "<class 'Topic.Topic'>":
    #         model.topics_models[i].set_coherence(coherences_per_topic[i])

    outfile = open(filename_persistence,'wb')
    pickle.dump(model,outfile)
    outfile.close()

    return filename_persistence, new_name, training_time
...
def train_save_submodels(route_to_model, route_to_persistence, model_name, model_for_expansion, selected_topic, nr_topics, version, thr):
    ...
    for i in np.arange(0,len(submodels_paths),1):
        ...
        training_time = timer.time() - start_time
        submodel.set_training_time(training_time)
        
        # submodel_corpus = pathlib.Path(new_submodel_path) / "submodel.txt"
        # corpus = []
        # with open(submodel_corpus, 'rb') as f:
        #     for myline in f: 
        #         a = str(myline).split("0 ")[1].split("\\")[0].split(" ")
        #         corpus.append(a)
        # submodel.set_corpus(corpus)
        
        # Get the topic words from the model
        # topics = []
        # for i in submodel.topics_models:
        #     if str(type(i)) == "<class 'Topic.Topic'>":
        #         topic = i.description[0][0].split(" ")
        #         topic = topic[0:len(topic)-1]
        #         topic_ok = []
        ...
        # coherence = cm.get_coherence()
        # coherences_per_topic = cm.get_coherence_per_topic()
        # print("coherences_per_topic submodel", coherences_per_topic)
        # print("avg cohr submodel", coherence)
        
        # submodel.set_avg_coherence(coherence)
        
        # for i in range(len(submodel.topics_models)):
        #     if str(type(submodel.topics_models[i])) == "<class 'Topic.Topic'>":
        #         submodel.topics_models[i].set_coherence(coherences_per_topic[i])
...
for num_topics in num_topics_root:
    # 1. Create new HTM (folder of the root model)
    model_path, model_name = create_model()
    
    # 2. Train root model
    route_to_persistence, model_name, training_time = train_model(model_path, model_name, num_topics)
    print("Training root model with nr topics = ", num_topics)
    # mes = "root, 0 " + str(num_topics) + " , " + str(training_time)
    # logger.debug(mes)
        ...
        for i in np.arange(0, len(model.topics_models), 1):
            if str(type(model.topics_models[i])) == "<class 'Topic.Topic'>":
                ...
                for thr in np.arange(0.1, 1, 0.1):
                    training_time = train_save_submodels(model_path, route_to_persistence, model_name, model_name, topic_id, num_topics_sub[j], "v2", thr)
                    # mes = "v2, " +  "{:.1f}".format(thr) + " , " +  str(num_topics_sub[j]) + " , " + str(training_time)
                    # logger.debug(mes)
                training_time = train_save_submodels(model_path, route_to_persistence, model_name, model_name, topic_id, num_topics_sub[j], "v1", "")
                # mes = "v1, 0, " +  str(num_topics_sub[j]) + " , " + str(training_time)
                # logger.debug(mes)
    ...
    #     print("directory removed: ", model_path)