...
def generatePyLavis(model_to_plot_str):
    ...
    ldamallet = LdaMallet(mallet_path, corpus=corpus_bow, num_topics=int(num_topics), id2word=dictionary,
                          workers=1)  # workers=1
...
def plot_diagnostics(list_diagnostics_id, measurement, measurement2, xaxis, yaxis, title, figure_to_save):
    ...
    valueY = ""
    plt.figure()
    print("plot diagnostics is called")
    model_names = []
    for el in list_diagnostics_id:
        ...
        if measurement2 == "topics":
            ...
            tokens = [child.get("tokens") for child in root if child.tag == 'topic']
            tokens_f = list(map(float, tokens))
            valueY = sum(tokens_f) / len(tokens_f)
        all_topics_measures = [child.get(measurement) for child in root if child.tag == 'topic']
        all_topics_measures_f = list(map(float, all_topics_measures))
        measure_avg = sum(all_topics_measures_f) / len(all_topics_measures_f)

        x.append(float(valueY))
        y.append(float(measure_avg))

    for i in np.arange(0, len(x), 1):
        plt.plot(x[i], y[i], 'o', label=model_names[i], markersize=10)
    plt.title(title)
    plt.xlabel(xaxis)
    plt.ylabel(yaxis)
    plt.legend()
